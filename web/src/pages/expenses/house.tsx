...
import { useHouseExpensesQuery } from "../../generated/graphql";
...
const Index: React.FC = () => {
  const { data, loading, fetchMore } = useHouseExpensesQuery({
    variables: { limit: 20 },
    ...
    if (loaderRef.current) {
      ...
      if (space >= -100 && !loading && doOnce.current) {
        ...
        await fetchMore({
          variables: {
            limit: 15,
            offset: data?.houseExpenses.expenses?.length,
          },
        });
        doOnce.current = true;
      }
    }
  }, [data?.houseExpenses.expenses?.length, fetchMore, loading]);
  ...
  if (data?.houseExpenses.auth?.includes("not authenticated")) {
    return <Login />;
  }
      ...
      {isDesktop ? (
        <>
          <Table variant="simple" w="100%">
            ...
            <Tbody>
              {data.houseExpenses.expenses?.map((expense) => (
        ...
        <>
          <Table variant="simple" colorScheme="teal" w="100%">
            <Tbody>
              {data.houseExpenses.expenses?.map((expense, i) => (
                <ExpenseMobile
                  key={i}
                  expense={expense as Expense}
                  prevExpense={
                    (data.houseExpenses.expenses as Expense[])[i - 1]
                  }
                />
              ))}
            </Tbody>
