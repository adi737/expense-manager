...
import { useMedicalExpensesQuery } from "../../generated/graphql";
...
const Index: React.FC = () => {
  const { data, loading, fetchMore } = useMedicalExpensesQuery({
    variables: { limit: 20 },
    ...
    if (loaderRef.current) {
      ...
      if (space >= -100 && !loading && doOnce.current) {
        ...
        await fetchMore({
          variables: {
            limit: 15,
            offset: data?.medicalExpenses.expenses?.length,
          },
        });
        doOnce.current = true;
      }
    }
  }, [data?.medicalExpenses.expenses?.length, fetchMore, loading]);
  ...
  if (data?.medicalExpenses.auth?.includes("not authenticated")) {
    return <Login />;
  }
      ...
      {isDesktop ? (
        <>
          <Table variant="simple" w="100%">
            ...
            <Tbody>
              {data.medicalExpenses.expenses?.map((expense) => (
        ...
        <>
          <Table variant="simple" colorScheme="teal" w="100%">
            <Tbody>
              {data.medicalExpenses.expenses?.map((expense, i) => (
                <ExpenseMobile
                  key={i}
                  expense={expense as Expense}
                  prevExpense={
                    (data.medicalExpenses.expenses as Expense[])[i - 1]
                  }
                />
              ))}
            </Tbody>
