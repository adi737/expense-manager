...
import { useEducationExpensesQuery } from "../../generated/graphql";
...
const Index: React.FC = () => {
  const { data, loading, fetchMore } = useEducationExpensesQuery({
    variables: { limit: 20 },
    ...
    if (loaderRef.current) {
      ...
      if (space >= -100 && !loading && doOnce.current) {
        ...
        await fetchMore({
          variables: {
            limit: 15,
            offset: data?.educationExpenses.expenses?.length,
          },
        });
        doOnce.current = true;
      }
    }
  }, [data?.educationExpenses.expenses?.length, fetchMore, loading]);
  ...
  if (data?.educationExpenses.auth?.includes("not authenticated")) {
    return <Login />;
  }
      ...
      {isDesktop ? (
        <>
          <Table variant="simple" w="100%">
            ...
            <Tbody>
              {data.educationExpenses.expenses?.map((expense) => (
        ...
        <>
          <Table variant="simple" colorScheme="teal" w="100%">
            <Tbody>
              {data.educationExpenses.expenses?.map((expense, i) => (
                <ExpenseMobile
                  key={i}
                  expense={expense as Expense}
                  prevExpense={
                    (data.educationExpenses.expenses as Expense[])[i - 1]
                  }
                />
              ))}
            </Tbody>
